...
func readSffPalette(f io.ReadSeeker, offset int64, size uint32, ver2 byte) ([]uint32, error) {
	...
	for i := 0; i < len(pal); i++ {
		...
		// Get existing color data
		// If len(pal) > rawCount the rest will be 0's
		if i < rawCount {
		...
		if ver2 == 0 { // Version 2.0.0.0 only?
			if i == 0 {
	...
	if s.header.Ver0 != 1 {
		...
		for i := 0; i < int(s.header.NumberOfPalettes); i++ {
			...
			if old, ok := uniquePals[[...]uint16{gn_[0], gn_[1]}]; ok {
				...
				// Read palette data
				var err error
				pal, err = readSffPalette(f, int64(lofs+ofs), plSize, s.header.Ver2)
				if err != nil {
					return nil, err
				}
				idx = i
...
func loadCharPalettes(sff *Sff, filename string, ref int) error {
	...
	for headerIdx := 0; headerIdx < int(h.NumberOfPalettes); headerIdx++ {
		...
		if old, ok := uniquePals[[2]uint16{gn_[0], gn_[1]}]; ok {
			...
			var err error
			pal, err = readSffPalette(f, int64(lofs+ofs), plSize, sff.header.Ver2)
			if err != nil {
				return err
			}
		}
			...
			if size == 0 {
				...
				if ok {
					...
					if h.Ver0 == 1 {
						...
						// Read palette data
						var err error
						spriteList[i].Pal, err = readSffPalette(f, int64(lofs+plXofs), plSize, h.Ver2)
						if err != nil {
							return nil, nil, err
						}
